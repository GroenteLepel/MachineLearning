...
for j in range(0, Niterations):
    ...
    for i in range(0, len(X)):
        r1[i] = pi1 * multivariate_normal.pdf((X[i], Y[i]), mean=mu1, cov=S1) /\
                (
                        pi1 * multivariate_normal.pdf((X[i], Y[i]),
                                                      mean=mu1,
                                                      cov=S1)
                        + pi2 * multivariate_normal.pdf((X[i], Y[i]),
                                                        mean=mu2,
                                                        cov=S2)
                )
        r2[i] = pi2 * multivariate_normal.pdf((X[i], Y[i]), mean=mu2,cov=S2) /\
                (
                        pi1 * multivariate_normal.pdf((X[i], Y[i]),
                                                      mean=mu1,
                                                      cov=S1)
        ...
        Snew1 = Snew1 + r1[i] * np.array(
            [[X[i] - mu1[0]], [Y[i] - mu1[1]]]) * np.array(
            [X[i] - mu1[0], Y[i] - mu1[1]])
        Snew2 = Snew2 + r2[i] * np.array(
            [[X[i] - mu2[0]], [Y[i] - mu2[1]]]) * np.array(
            [X[i] - mu2[0], Y[i] - mu2[1]])
