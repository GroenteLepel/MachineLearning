    ...
    if ncols == 1:
        ...
        for ax0 in ax[0]:
            ax0.hlines(y=0, xmin=-10,
                       xmax=1e4,
                       colors='grey', linestyles='dotted')
        for axn in ax[:][nrows - 1]:
            axn.set_xlabel("Temperature")
...
def plot_sa(t_values, mean_energies, std_energies, show:  bool = True):
    fig, ax = init_plot(2, 1)

    ax[0].plot(t_values, mean_energies, c="black")
    ax[1].plot(t_values, std_energies, c="black")

    ax[0].set_xlim(t_values.min(), t_values.max())
    ...
    if show:
        ...
        print("Saved figure at {}{}.".format(datafolder, fname))
...
def three_columns(x_data, y1_data, y2_data, method: str, show=True):
    ...
    if show:
        ...
        path = "../data/paramtest_{}.png".format(method)
        plt.savefig(path)
        print("Saved figure at {}.".format(path))
